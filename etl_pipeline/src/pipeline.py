...
class Pipeline:
    ...
    def run(self):
        ...
        # Determine source type from config or default to file
        # In a real app, this might be more dynamic. Here we check keys.
        if 'hive' in source_config and source_config['hive'].get('table_name'):
            source_type = 'hive'
            s_conf = source_config['hive']
        else:
            source_type = 'file'
